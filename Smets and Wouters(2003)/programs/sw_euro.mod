var MC E EF R_KF QF CF IF YF LF PIEF WF RF R_K Q C I Y L PIE W R EE_A PIE_BAR EE_B EE_G EE_L EE_I KF K one;    
...
model(linear); 
	      ...
	      Q = -(R-PIE(1))+(1-beta*(1-tau))*((1+czcap)/czcap)*R_K(1)+beta*(1-tau)*Q(1) +EE_I*0+0*ETA_Q ;
	      K =  (1-tau)*K(-1)+tau*I(-1) ;
...
estimation(datafile=rawdata_euromodel_1,presample=40, first_obs=1, nobs=118, lik_init=2, mode_compute=0,mode_file=sw_euro_mode,mh_replic=0);
